...
WORDLIST_FILENAME = "G:/Python_work/ProblemSet4/words.txt"
...
def getWordScore(word, n):
    ...
    assert type(n) == int, "Error: type(n) != int"
    assert n > 0, "Error: n <= 0"
    assert len(word) <= n, "Error: len(word) > n"
    score = 0
    for char in word:
    ...
    assert type(score) == int, "type(score) != n"
    return score
...
def isValidWord(word, hand, wordList):
    ...
    copy = hand.copy()
    for char in word:
        if (copy.get(char, 0) == 0):    # if word is not composed of letters in hand
            return False
        copy[char] = copy.get(char, 0) - 1
    if word in wordList:
        return True
    ...
    while (calculateHandlen(hand) > 0):
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                total += score
                print '"'+inputWord+'" earned '+str(score)+' points. Total: '+str(total)+' points'
                print
                # Update the hand 
